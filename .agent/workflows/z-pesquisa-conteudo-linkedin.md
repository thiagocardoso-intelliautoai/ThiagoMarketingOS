...
1. Salvar o post aprovado em arquivo markdown: `aiox-squads/squads/pesquisa-conteudo-linkedin/output/post-final.md`
   - O arquivo DEVE começar com front-matter YAML (REFAC-002):
     ```yaml
     ---
     title: "..."
     pillar: A
...
> **Nota:** O CLI parseia automaticamente o markdown para extrair hook, body, CTA, fontes, metadata
> e o front-matter de lead-magnet (REFAC-002).
> É idempotente — pode rodar múltiplas vezes sem duplicar dados (upsert por slug).

### Override manual de lead magnet via flags (REFAC-002)

Caso o front-matter esteja ausente OU você queira sobrescrever, use os flags:

```bash
# Marcar manualmente como lead magnet
node aiox-squads/shared/scripts/save-post-cli.js \
  --title "TITULO" --file output/post-final.md --pillar A --review-score 87 \
  --is-lead-magnet \
  --lead-magnet-resource "framework de prospecção em 5 etapas" \
  --cta-arte "Comente FRAMEWORK pra receber na DM"

# Forçar como NÃO lead magnet (override do front-matter)
node aiox-squads/shared/scripts/save-post-cli.js \
  --title "TITULO" --file output/post-final.md --no-is-lead-magnet
```

Precedência: flag CLI > front-matter > default (`is_lead_magnet=false`).
